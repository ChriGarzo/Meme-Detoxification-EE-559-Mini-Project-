...
\textbf{Teacher pseudo-labels.} For each split, LLaVA-NeXT first generates a
JSON explanation with three fields: target group, visual evidence, and implicit
...
\textbf{BART student.} The student starts from BART-large and is trained only
on the meme pseudo-rewrite data, without ParaDetox warm-up.
...
to BART attention projections and feed-forward layers
(\texttt{q,k,v,out\_proj,fc1,fc2}) with rank 32, alpha 64, dropout 0.05,
...
text-only null-context prompt, then decoded by the fine-tuned BART decoder. This
...
metrics and does not reproduce harmful images or captions except through
sanitized task descriptions.
...
pseudo-rewrites, a single reported DetoxLLM-7B run, and base BART. VisualBERT
hate probability was also computed, but its non-hateful prediction rate was 0
for every system, so we do not use it as a main conclusion.
...
$^\dagger$DetoxLLM is a single reported run. Text \sta{} is mean non-toxic
probability from RoBERTa; \sta{} $\Delta$ is rewrite \sta{} minus original
\sta{}; SIM is BERTScore F1; CLIP is normalized CLIPScore. Higher is better.}
\label{tab:results}
\vskip -0.12in
\end{table}
...
reported DetoxLLM baseline. The full condition has the best average \bartft{}
detoxification, while \texttt{target\_only} is close and has the best BART-only
SIM. The ablation is most visible in variance: \texttt{full} and
\texttt{target\_only} are stable in SIM ($0.013$ standard deviation), whereas
\texttt{visual\_only} has much larger SIM variance ($0.080$). Visual evidence
alone is therefore not a reliable control signal for a text rewriter.
...
\paragraph{Limitations.} The project depends on LLaVA pseudo-labels rather than
...
fields, not pixels, and VisualBERT pass/fail was uninformative on this test set.
