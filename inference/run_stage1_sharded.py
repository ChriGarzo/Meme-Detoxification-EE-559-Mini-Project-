...
def _compute_text_toxicity(
    ...
    """Return toxicity probability (class 1) for each text using the RoBERTa classifier.

    Mirrors the compute_sta_score logic from run_stage1.py so that passed_stage1_filters
    is computed consistently with the train set.
    """
    if not texts:
        return []
    out: List[float] = []
...
def main():
    ...
    # Load text STA model (RoBERTa toxicity classifier) for pass/fail quality filter.
    # Using text STA (not visual) keeps passed_stage1_filters consistent with train behaviour.
        ...
        with tqdm.tqdm(total=len(records), desc="Processing examples") as pbar:
            for start_idx in range(0, len(records), args.batch_size):
                ...
                if rewrite_positions:
                    ...
                    # Text toxicity for original (used for toxicity_drop, consistent with train)
                    rw_original_toxicities = _compute_text_toxicity(
                        rw_original_texts, _sta_model, _sta_tok, device
                    )
                    # Visual toxicity for original (annotation only)
                    rw_visual_original_toxicities = compute_multimodal_hatefulness(
                        rw_image_paths,
                        rw_original_texts,
                        hate_scorer,
                    ...
                    if kept_slots:
                        ...
                        # Text toxicity for rewrite (used for sta_score and toxicity_drop)
                        ...
                        # Visual toxicity for rewrite (annotation only)
                        visual_rewrite_toxicities = compute_multimodal_hatefulness(
                            kept_image_paths,
                            kept_rewrite_texts,
                            hate_scorer,
                            batch_size=args.score_batch_size,
                        ...
                        for slot_idx, rewrite, sta_score, rewrite_toxicity, visual_rewrite_tox, bertscore, original_toxicity, visual_original_tox in zip(
                            kept_slots,
                            kept_rewrite_texts,
                            sta_scores,
                            rewrite_text_toxicities,
                            visual_rewrite_toxicities,
                            bertscores,
                            kept_original_toxicities,
                            [rw_visual_original_toxicities[i] for i in kept_slots],
                        ):
                            ...
                            # Write ALL records (passed + failed) with passed_stage1_filters field,
                            # consistent with the train set behaviour.
                            rewrite_record = {
                                "id": example_id,
                                ...
                                "multimodal_original_toxicity": float(visual_original_tox),
                                "multimodal_toxicity": float(visual_rewrite_tox),
                                "multimodal_sta_score": float(1.0 - visual_rewrite_tox),
