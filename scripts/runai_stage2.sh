...
#
# Usage:
#   bash scripts/runai_stage2.sh <UID_NUMBER>   # personal home code path
#   bash scripts/runai_stage2.sh                # scratch code path
#
...
#   INPUT_FORMAT=explicit_detox OUTPUT_SUFFIX=_explicit_detox bash scripts/runai_stage2.sh <UID>
#     writes checkpoints like /scratch/hmr_stage2_full_explicit_detox_checkpoint
# =============================================================================
...
TRAIN_SCRIPT="${CODE_ROOT}/training/train_stage2.py"
if [ ! -f "${TRAIN_SCRIPT}" ]; then
    if [ "${MODE_LABEL}" = "scratch" ] && [ -f "${REPO_ROOT_LOCAL}/training/train_stage2.py" ]; then
...
echo "=== Stage 2: BART LoRA Meme Fine-tune ==="
...
for CONDITION in "${CONDITIONS[@]}"; do
    ...
    runai submit hmr-stage2-${SAFE_CONDITION}${JOB_SUFFIX} \
        --run-as-uid ${UID_NUM} \
        --image ${IMAGE} \
        --node-pools a100-40g \
        --gpu 1 \
        ...
        --command -- python3 ${TRAIN_SCRIPT} \
            ...
            --output_dir /scratch/stages/hmr_stage2_${CONDITION}${OUTPUT_SUFFIX}_checkpoint \
